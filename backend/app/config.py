...
class Settings(BaseSettings):
    ...
    # ── External APIs ─────────────────────────────────────────────
    firms_api_key: str = Field(
        default="",
        validation_alias=AliasChoices("FIRMS_MAP_KEY", "FIRMS_API_KEY"),
    )
    ...
    poll_interval_minutes: int = Field(
        default=10,
        validation_alias=AliasChoices("INGEST_INTERVAL_MINUTES", "POLL_INTERVAL_MINUTES"),
    )

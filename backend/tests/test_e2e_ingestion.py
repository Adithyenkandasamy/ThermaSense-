...
Uses isolated in-memory SQLite test database.
"""
    ...
    for i in range(start_id, start_id + count):
        hotspots.append(
            HotspotResponse(
                ...
                latitude=round(34.0 + (i * 0.001), 6),
                longitude=round(-118.0 + (i * 0.001), 6),
...
@pytest.mark.asyncio
async def test_end_to_end_deduplication_and_logging(client: AsyncClient):
    """
    Verify complete deduplication cycle in isolated database:
    ...
    with patch("app.services.observation_service.fetch_hotspots", new=AsyncMock(return_value=(batch_1, "VIIRS_NOAA20_NRT", area_tag))):
        res1 = await client.post("/api/ingestion/firms", json={
    ...
    with patch("app.services.observation_service.fetch_hotspots", new=AsyncMock(return_value=(batch_1, "VIIRS_NOAA20_NRT", area_tag))):
        res2 = await client.post("/api/ingestion/firms", json={
            "source": "VIIRS_NOAA20_NRT",
            "area": area_tag,
            "day_range": 1,
        })
    ...
    with patch("app.services.observation_service.fetch_hotspots", new=AsyncMock(return_value=(batch_3, "VIIRS_NOAA20_NRT", area_tag))):
        res3 = await client.post("/api/ingestion/firms", json={
            "source": "VIIRS_NOAA20_NRT",
            "area": area_tag,
            "day_range": 1,
        })
    ...
    logs_res = await client.get("/api/monitoring/logs", params={"source": "VIIRS_NOAA20_NRT", "limit": 10})
    ...
    status_res = await client.get("/api/monitoring/status")
